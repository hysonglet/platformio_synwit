...
#define DRAW_BUFF_CNT      2
    ...
    lv_color_t *buf_1;
    buf_1 = lv_mem_alloc(sizeof(lv_color_t)*MY_DISP_HOR_RES*MY_DISP_VER_RES);
    lv_disp_draw_buf_init(&draw_buf_dsc_1, buf_1, NULL, MY_DISP_HOR_RES*MY_DISP_VER_RES);   /*Initialize the display buffer*/
#elif DRAW_BUFF_CNT == 2
    /* Example for 2) */
    static lv_disp_draw_buf_t draw_buf_dsc_2;
    // static lv_color_t buf_2_1[MY_DISP_HOR_RES * 10];                        /*A buffer for 10 rows*/
    // static lv_color_t buf_2_2[MY_DISP_HOR_RES * 10];                        /*An other buffer for 10 rows*/
    lv_color_t *buf_2_1, *buf_2_2;
    buf_2_1 = lv_mem_alloc(sizeof(lv_color_t)*MY_DISP_HOR_RES*MY_DISP_VER_RES);
    buf_2_2 = lv_mem_alloc(sizeof(lv_color_t)*MY_DISP_HOR_RES*MY_DISP_VER_RES);
...
{
    /*You code here*/
    rgb_lcd_init(LCD_BUFF_ADDR, LCD_WIDTH, LCD_HEIGHT);
}
    ...
    if(disp_flush_enabled) {
        ...
        for(y = area->y1; y <= area->y2; y++) {
            for(x = area->x1; x <= area->x2; x++) {
                ...
                /*put_px(x, y, *color_p)*/
                lcd_pixel_set_565(LCD_BUFF_ADDR, x, y, color_p->full);
                color_p++;
            }
        }
    }
